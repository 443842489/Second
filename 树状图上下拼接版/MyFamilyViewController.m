...
-(void)updateView:(MyMemberModel*)entity
{
    ...
    self.maxWidth=upMaxWidth>downMaxWidth?upMaxWidth:downMaxWidth;
...
//       [self addCell:self.fModel x:XSpace y:MainScreenH-NavigationHeight-HeadHeight-YSpace-50];
    ...
    if(upMaxWidth>downMaxWidth)//如果上面的宽度大于下面的宽度,那就隐藏下面部分的root单元，以上面宽度的X中心坐标+Xspace 作为上下两边root单元的X中心坐标
    {
        ...
        [self addDownCell:self.fModel x:x+self.maxWidth/2-downMaxWidth/2 y:y+1];//添加下面分支
//    [self addDownCell:self.fModel x:x+self.maxWidth/2-downMaxWidth/2 y:y+HeadHeight+YSpace];
    }else{
...
//        for (int i= 0 ; i < entity.children.count ; i++) {
//            //        float num=y-YSpace-HeadHeight;
//            float x1=sh2-(entity.children.count)*(HeadWidth+XSpace)/2;
//            float y1=y+YSpace+HeadHeight;
//            NSLog(@"x%f  y%f",x1,y1);
//            MyMemberModel *model;
//            MyMemberModel *smallModel;
//            model=entity.children[i];
//            if(model.children.count>0)
//            {
//                smallModel=model.children[i];
//            }
//            float subcellX2 = [self addDownCell:smallModel x:x1-(entity.children.count)*(HeadWidth+XSpace)/2 y:y1+YSpace+HeadHeight];
//            
//            maxwidth =[self getWidth:entity.children[i] isUp:NO];
//            MemberRectView *rectView=[[MemberRectView alloc]initWithMemberModel:entity];
//            rectView.menberView.lb_nick.text=model.title;
//            rectView.frame=CGRectMake(x1,y1, maxwidth, HeadHeight+YSpace);
//            rectView.backgroundColor=[UIColor redColor];
//            [self.view_main.scrollview addSubview:rectView];
//            
//            if (i == 0) {
...
//        {
//    
//            View *yBranchLine=[[View alloc]initWithFrame:CGRectMake(bottomX2, y+HeadHeight+YSpace-0.5,bottomX2-topX2, 1)];
//            yBranchLine.backgroundColor=[UIColor blueColor];
//            
//            [self.view_main.scrollview addSubview:yBranchLine];
//        }
//    
//    return maxwidth/2+x;
    ...
    //上一个cell 的中点x
    //上一个cell 的y
    //上一个cell 的宽度：maxwidth =[self getWidth:entity isUp:NO];
    //本cell 的宽度 myWidth =[self getWidth:entity.children[i] isUp:NO];
    //求 下一个cell的 中心x ，y
    
    
//    for(int i=0;i<entity.children.count;i++)
//    {
//        float subcellX2=[self addDownCell:entity.children[i] x:x y:<#(float)#>]
//        
//        
//    }
